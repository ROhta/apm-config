...
- **即時 1 回**: push 完了から約 2 分 (120 秒) 待機 (レビュー bot〈Copilot / CodeRabbit 等〉の初回反応待ち) し、§2 の検知を 1 回実行する。
- **追跡**: 指摘は遅延することがあるため、さらに約 2 分後にもう 1 回検知を再実行する。実行環境にスケジュール機構があればそれで予約し、なければ手動で約 2 分後に §2 を再実行する。即時 + 追跡で **連続 2 回** 新規指摘がなければ追跡を終了する。
  - 予約手段の例 — Claude Code の `ScheduleWakeup`:

    ```text
    ScheduleWakeup({ delaySeconds: 120,
      prompt: "PR #<番号> のレビュー応答ループを再開する。review-response-loop ルールに従い、未 resolve スレッドを検知して処理せよ。",
      reason: "レビュー bot 遅延応答の追跡チェック (push から 2 分後)" })
    ```
